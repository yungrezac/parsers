...
// Fetch через прокси (Netlify Function)
...
async function fetchSearchProducts(query) {
    ...
    while (true) {
        ...
        data.data.products.forEach(p => {
            let stock = 0;
            if (p.sizes) p.sizes.forEach(s => { if (s.stocks) s.stocks.forEach(st => stock += st.qty || 0); else stock += s.qty || 0; });
            allProducts.push({
                id: p.id,
                name: p.name,
                brand: p.brand,
...
async function fetchMassProducts(sellerId, brandId) {
    ...
    let endpoint = '';
    if (sellerId) endpoint = `https://catalog.wb.ru/sellers/v4/catalog?appType=1&curr=rub&dest=-1257786&sort=popular&spp=30&supplier=${sellerId}`;
    else if (brandId) endpoint = `https://catalog.wb.ru/brands/v2/catalog?appType=1&curr=rub&dest=-1257786&sort=popular&spp=30&brand=${brandId}`;
    const allProducts = [];
    let page = 1;
    const maxPages = 10;
    while (true) {
...
document.addEventListener('DOMContentLoaded', () => {
    try { Telegram.WebApp.ready(); Telegram.WebApp.expand(); } catch (e) { console.error("Ошибка TWA:", e); }
    ...
    function chartOptions(scales = {}) {
        return {
            ...
            scales: {
                y: { beginAtZero: true, ticks: { color: 'var(--tg-hint)' }, grid: { color: 'var(--tg-secondary-bg)' }, ...scales.y },
                x: { ticks: { color: 'var(--tg-hint)' }, grid: { display: false }, ...scales.x }
            }
        };
    }
    ...
    exportBtn.addEventListener('click', () => {
        ...
        parsedData.forEach(p => { if (p.params) Object.keys(p.params).forEach(k => allParamKeys.add(k)); });
        const paramKeysArray = Array.from(allParamKeys);
        const flatData = parsedData.map(p => {
            const base = { "Артикул": p.id, "Название": p.name, "Бренд": p.brand, "Цена": p.price, "Рейтинг": p.rating, "Отзывы": p.reviews, "Остаток": p.stock };
            paramKeysArray.forEach(key => base[key] = p.params ? (p.params[key] || "") : "");
            return base;
        });
    ...
    function setLoading(isLoading, btnElement, btnText = "") {
        ...
        if (btnElement) {
            ...
            btnElement.innerHTML = isLoading ? `<div class="loader"></div>` : `<span>${btnText}</span>`;
        }
    }
